...
/* Isochronous endpoints limitations: only full speed is
...
void Delay_USB(volatile unsigned count) {
  while (count--);
}
...
static int USBD_periphDeInit(void) {
  ...
  dctl.d32 = 0;
  dctl.b.sdis = 1;
  FS_USB_OTG_DREGS->DCTL = dctl.d32;

  return 0;
}

static int USBD_periphInit(int clk) {
  USB_OTG_DCFG_TypeDef    dcfg;
